...
t_se =3.16e7# Sec   Time Period of Alpha Centauri
#NOTE t_ac = 80*t_se,t_se = 3.16e7 sec

# Scaling Constants
K1 = G * t_se * m_sun / (r_ac ** 2 * v_e)
K2 = v_e * t_se / r_ac
link = 'www.github.com/'
# Function for 2-body motion equations
...
# Function to compute the center of mass position and velocity
...
def com_vel(M1, V1, M2, V2):
    ...
    return com_v
#Importing Saved Parameters file
try:
    parameters = pd.read_csv('Saved_Parameters_1.csv')
    ...
    quit()
choice = 100
print("NOTE: All the data are in terms of Solar Mass, Orbital Velocity of Earth, Distance b/w Alpha Centauri, Time period of Alpha Centauri system(~80 years), and should be entered likewise only for good performance.")
...
while choice != 5:
    ...
    if choice == 1:
        ...
        h1 = [ax.scatter(r1_sol[0,0],r1_sol[0,1],r1_sol[0,2],color="darkblue",marker="o",s=80,label=body_1)]
        h2 = [ax.scatter(r2_sol[0,0],r2_sol[0,1],r2_sol[0,2],color="tab:red",marker="o",s=80,label=body_2)]
        ax.legend()
        def anim(i):
            h1[0].remove()
            h2[0].remove()
            t1 = ax.plot(r1_sol[:i,0],r1_sol[:i,1],r1_sol[:i,2],color = 'b')
            t2 = ax.plot(r2_sol[:i,0],r2_sol[:i,1],r2_sol[:i,2],color = 'r')
        ...
        animation = animation.FuncAnimation(fig,anim,frames=fra,interval=1)
        save=input('Do you want to save sim?(y/n):')
        if save.lower() == 'y':
            name = input('Enter file name:')
            name = name+'.mp4'
            print(f"Animation will be saved as {name}")
            animation.save(name, writer='ffmpeg',dpi=300,fps=30)
        ...
        continue_editing = input("Do you want to continue editing(y/n):")
